...
class MainActivity : AppCompatActivity() {
    ...
    private fun getAirQualityData(latitude: Double, longitude: Double) {
        ...
        retrofitAPI.getAirQaulityDate(
            latitude.toString(),
            longitude.toString(),
            "979e2b94-eadd-47a8-a7e8-ad35fd23c8cc"
            ...
            override fun onResponse(
                ...
                if(response.isSuccessful){
                    ...
                    "최신정보 업데이트 완료!",Toast.LENGTH_SHORT).show()
    ...
    private fun updateAirUI(airQualityData:AirQualityResponse){
        ...
        val dateFormater:DateTimeFormatter=DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")
        binding.tvCheckTime.text = dateTime.format(dateFormater).toString()

        when(pollutionData.aqius){
            in 0..50 ->{ //대기지수
                binding.tvTitle.text="좋음"
                binding.imaBg.setImageResource(R.drawable.bg_good)
            }
            in 51..150 ->{
                binding.tvTitle.text="보통"
                binding.imaBg.setImageResource(R.drawable.bg_soso)
            }
            in 151..200 ->{
                binding.tvTitle.text="나쁨"
                binding.imaBg.setImageResource(R.drawable.bg_bad)
            }
            else ->{
                binding.tvTitle.text="매우 나쁨"
                binding.imaBg.setImageResource(R.drawable.bg_worst)
            }
        }
    }
